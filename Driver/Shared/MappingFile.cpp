...
void MappingFile::runCountIncrease()
{
    if (LSData == nullptr) {
        LogE("MappingFile.runCountIncrease():LSData值为null,直接返回!");
        return;
    }

    LSData->runCount = LSData->runCount + 1;
}

void MappingFile::writeLux(float lux)
{
    if (LSData == nullptr) {
        LogE("MappingFile.writeLux():LSData值为null,直接返回!");
        return;
    }
...
{
    if (LSData == nullptr) {
        LogE("MappingFile.readLux():LSData值为null,直接返回!");
        return 0;
    }
